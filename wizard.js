...
const updateSubmitFormButtonDisabledStatus = () => {
  ...
  if( submitFormButton.disabled ) {
    ...
    submitFormButton.classList.add( 'seethrough' )
  } else {
    submitFormButton.classList.remove( 'gray' )
    submitFormButton.classList.remove( 'seethrough' )
    
    submitFormButton.classList.add( 'success' )
    submitFormButton.classList.add( 'white-text' )
  }
}

textInput.addEventListener(
  'input',
  ( event ) => { refreshVariablesButton.click() }
)
...
story.addEventListener(
  ...
  ( event ) => {
    ...
    const extractedVariables = ( userInput.match( variablePattern ) || [ ])
      .map( match => match.replace( /(?:\{|\})/g, '' ).replace( /\s+/, '' ) )
